...
that the variable uses the `labelled` type. The function `is.labelled()` will also
tell you if a variable is labelled.

By default the ipumsr package now will attempt to show the value labels when
printing out a data.frame (like the `"[Wis~"` on STATEFIP above, for Wisconsin). 
...
However, as you can see, there often isn't enough space to see the labels 
when printing out a full data.frame. So, there are a few options to see the 
labels directly:
```{r}
...
let us know by filing an issue on [github](https://github.com/ipums/ipumsr/issues).
